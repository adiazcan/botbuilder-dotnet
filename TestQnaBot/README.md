...
# To try this sample
- In a terminal, navigate to `<your_project_folder>/TestQnaBot`
    ```bash
    # change into project folder
	cd <your_project_folder>/TestQnaBot
    ```
- Setup LUIS
    ...
    - As the command `msbot clone services` uses secret-manager to safely store the key generated for encrypting the bot file, it's necessary adding the next code in your TestQnaBot.csproj file:
    ...
    **Note**: You can add any arbitrary `GUID` value for the `UserSecretsId` property, but you can not repeat `GUID` values among the projects, as they must be unique.
    ...
    msbot clone services --name "TestQnaBot" --luisAuthoringKey <LUIS-authoring-key> --code-dir "." --location westus --sdkLanguage "CSharp" --folder deploymentScripts/msbotClone --verbose
    ```

    **Note**: Once the Luis service application is created, change the value of `LuisConfiguration` variable in TestQnaBotBot.cs with the Luis service name, which can be found in TestQnaBot.bot file (it should be `TestQnaBot_core-bot-LUIS`).
...
- Run the bot from a terminal or from Visual Studio, choose option A or B.
	...
	- Navigate to `<your_project_folder>/TestQnaBot` folder
	- Select `TestQnaBot.csproj` file
...
msbot clone services --name "TestQnaBot" --code-dir "." --location westus --sdkLanguage "Csharp" --folder deploymentScripts/msbotClone --verbose
```
